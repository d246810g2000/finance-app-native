...
import {
  ...
  saveCustomAccountMappings,
  loadExcludedAccounts,
  saveExcludedAccounts as saveExcludedAccountsService,
} from '../services/accountConfigService';
import {
  loadBudgetConfig,
  saveBudgetConfig as saveBudgetConfigService,
  loadBudgets,
  saveBudgets as saveBudgetsService,
} from '../services/budgetService';
import {
  loadCreditCardSettings,
  saveCreditCardSettings as saveCreditCardSettingsService,
...
/** 帳戶／預算／信用卡等設定 — 變更時不應拖垮只訂閱 records 的畫面 */
export interface FinanceSettingsContextType {
...
export type FinanceContextType = FinanceRecordsContextType & FinanceSettingsContextType;
...
const FinanceRecordsContext = createContext<FinanceRecordsContextType | undefined>(undefined);
...
function FinanceDataProvider({ children }: { children: ReactNode }) {
  ...
  const [budgetConfig, setBudgetConfig] = useState<BudgetGlobalConfig>({
    includedProjects: [],
    splitProjects: [],
    projectGroups: {},
  });
  ...
  const mergeRecords = useCallback(
    (incoming: RawRecord[], options?: { syncDelete?: boolean }): UpsertResult => {
  ...
  const updateRecords = useCallback(
    (updates: Array<{ id: string; patch: Partial<RawRecord> }>) => {
  ...
  return (
    <FinanceSettingsContext.Provider value={settingsValue}>
      <FinanceRecordsContext.Provider value={recordsValue}>{children}</FinanceRecordsContext.Provider>
...
/** 只訂閱 records／轉換／CRUD（設定變更不會重渲染） */
export const useFinanceRecords = () => {
  const context = useContext(FinanceRecordsContext);
  if (!context) {
    throw new Error('useFinanceRecords must be used within a FinanceProvider');
  }
  return context;
};

/** 只訂閱帳戶／預算／信用卡設定（records 變更不會重渲染） */
export const useFinanceSettings = () => {
  const context = useContext(FinanceSettingsContext);
  if (!context) {
    throw new Error('useFinanceSettings must be used within a FinanceProvider');
  }
  return context;
};

/**
 * 同時訂閱 records + 設定。能拆則優先用 useFinanceRecords / useFinanceSettings，
 * 減少無關 state 變更造成的重渲染。
 */
export const useFinance = (): FinanceContextType => {
  const records = useFinanceRecords();
  const settings = useFinanceSettings();
  return useMemo(() => ({ ...records, ...settings }), [records, settings]);
};

/** 同時需要 Data + UI 時使用；會訂閱兩邊變更 */
